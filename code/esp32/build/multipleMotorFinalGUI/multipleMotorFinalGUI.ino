      ...
      if (client.available()) {
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            client.println("<input type='range' id='masterSlider' class='slider' min='-5000' max='5000' value='0' oninput='updateMaster()'>");
            client.println("<span id='masterValue'>0</span>");
            client.println("<input type='number' id='masterInput' value='0' onchange='updateMasterFromInput()'>");
            ...
            for (int i = 0; i < NMOTORS; i++) {
              ...
              client.print(i);
              client.print("' class='slider' min='-5000' max='5000' value='");
            ...
            client.println("<div>");
            client.println("<button onclick='startProgram()'>Start</button>");
            client.println("<button onclick='stopProgram()'>Stop</button>");
            client.println("<button onclick='resetProgram()'>Reset</button>");
            client.println("</div>");
            ...
            client.println("function startProgram() {");
            client.println("  fetch('/control?action=start');");
            client.println("}");

            client.println("function stopProgram() {");
            client.println("  fetch('/control?action=stop');");
            client.println("}");

            client.println("function resetProgram() {");
            client.println("  fetch('/control?action=reset');");
            client.println("  for (var i = 0; i < 5; i++) {");
            client.println("    document.getElementById('motor' + i).value = 0;");
            client.println("    document.getElementById('motorValue' + i).textContent = 0;");
            client.println("    document.getElementById('motorInput' + i).value = 0;");
            client.println("  }");
            client.println("  document.getElementById('masterSlider').value = 0;");
            client.println("  document.getElementById('masterValue').textContent = 0;");
            client.println("  document.getElementById('masterInput').value = 0;");
            client.println("}");
          ...
          if (currentLine.startsWith("GET /control?action=")) {
            String action = currentLine.substring(20, currentLine.indexOf(' ', 20));
...
void handleControl(String action) {
  if (action == "start") {
    ...
    // Stop all motors
    for (int k = 0; k < NMOTORS; k++) {
      setMotor(0, 0, pwm[k], in1[k]);
    }
    Serial.println("Program stopped");
  } else if (action == "reset") {
    programRunning = false;
    // Reset all encoder positions and targets to 0
    for (int k = 0; k < NMOTORS; k++) {
      posi[k] = 0;
      target[k] = 0;
      setMotor(0, 0, pwm[k], in1[k]);
